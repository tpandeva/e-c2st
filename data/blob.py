...
class BlobData(DatasetOperator):
    ...
    def __init__(self, type, samples, r=3, d=2, rho=0.03, with_labels=False, seed=0, tau1=None, tau2=None, rd=0.02):
        """
        ...
        for i in range(9):
            ...
            if i < 4:
                sigma_mx_2[i][0, 1] = -rd - rd * i/10
                sigma_mx_2[i][1, 0] = -rd - rd * i/10
            if i == 4:
                sigma_mx_2[i][0, 1] = 0.00
                sigma_mx_2[i][1, 0] = 0.00
            if i > 4:
                sigma_mx_2[i][1, 0] = rd + rd * (i-5)/10
                sigma_mx_2[i][0, 1] = rd + rd * (i-5)/10
...
class BlobDataGen(DataGenerator):
    ...
    def __init__(self, type, samples, data_seed, r, d, rho, with_labels,rd,proportion):
        """
        ...
        # 
        # Store parameters
        self.type, self.samples, self.data_seed, self.r, self.d, self.rho, self.with_labels,self.rd,self.proportion = type, samples, data_seed, r, d, rho, with_labels,rd,proportion

    def generate(self, seed, tau1, tau2, mode="train") -> Dataset:
        """
        ...
        if mode in ["val", "test"]:
            return BlobData(self.type, int(self.proportion*self.samples), self.r, self.d, self.rho, self.with_labels, modified_seed, tau1, tau2,self.rd)
        if mode=="train":
            return BlobData(self.type, int(self.samples), self.r, self.d, self.rho, self.with_labels, modified_seed, tau1, tau2,self.rd)